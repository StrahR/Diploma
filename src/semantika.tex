...
\begin{definition}
    ...
    \begin{enumerate}[(a)]
        ...
        \item ima množenje in enoto zanj,
        \item ima involucijo \((-)^*\),
...
\end{remark}
\begin{remark}
    Fiziki involucijo pogosto označujejo z \((-)^\dagger\) zaradi zgodovinskih razlogov.
\end{remark}
...
\begin{example}
    ...
    Res, lastnosti (a) do (c) držijo (to vemo od prej), lastnost (d) je pa preprosto preveriti: \(\det{A^{H}A} = \det{A^H}\det{A} = \p{\det{A}}²\).
\end{example}

\begin{definition}
    %TODO: finish definition
    Direktna vsota \(C^*\)-algeber \(X⊕Y\) je kartezičen produkt množic z operacijami definiranimi po komponentah.
\end{definition}
...
\begin{proposition}
    \(C*\)-algebre skupaj s popolnoma pozitivnimi eniškimi preslikavami tvorijo kategorijo \(\cstarcpu\) (angl. \begin{otherlanguage}{en}Completeley Positive Unital maps\end{otherlanguage}).
\end{proposition}
\begin{proof}
    TODO
\end{proof}
...
\begin{proof}
    ...
    \[0 = \op{new}{\bmat{0&1\\0&0}}\op{new}{\bmat{0&0\\1&0}}
